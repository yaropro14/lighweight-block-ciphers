...
int main(int argc, char **argv) {
  ...
  f = fopen(argv[1], "r");
  nread = fread(data, sizeof(char), data_size, f);
  fclose(f);
  ...
  char *cipher = (char*)malloc(sizeof(char) * data_size);
  ...
  f = fopen(argv[2], "r");
  nread = fread(cipher, sizeof(char), data_size, f);
